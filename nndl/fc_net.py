...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
            # Affine forward
            scores, cache = affine_forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
            layer_caches.append(cache)

            # Batchnorm forward (if applicable)
            if self.use_batchnorm:
                scores, cache = batchnorm_forward(scores, self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], self.bn_params[i])
                layer_caches.append(cache)
    ...
    reg_loss = sum(0.5 * self.reg * np.sum(W**2) for W in self.params.values())
    loss = data_loss + reg_loss

        # Backward pass
    dout, dW, db = affine_backward(dscores, layer_caches.pop())
    grads[f'W{self.num_layers}'] = dW + self.reg * self.params[f'W{self.num_layers}']
    grads[f'b{self.num_layers}'] = db

    for i in reversed(range(self.num_layers - 1)):
            # Dropout backward (if applicable)
            if self.use_dropout:
                dout = dropout_backward(dout, layer_caches.pop())

            # ReLU backward
            dout = relu_backward(dout, layer_caches.pop())

            # Batchnorm backward (if applicable)
            if self.use_batchnorm:
                dout, dgamma, dbeta = batchnorm_backward(dout, layer_caches.pop())
                grads[f'gamma{i+1}'] = dgamma
                grads[f'beta{i+1}'] = dbeta

            # Affine backward
            dout, dW, db = affine_backward(dout, layer_caches.pop())
